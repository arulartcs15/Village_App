...
const Container = (props) => {
    ...
    const getRandomColor = () => {
        const color = ['gold', 'orange', 'violet']
        let color1
        for (let i = 0; i < color.length; i++) {
            let a = Math.floor(Math.random() * (color.length + 1))
            color1 = color[a]
        }
        return color1
    };
                ...
                isLogged && data.role !== 'superAdmin' && data.role !== 'resident' ?
                    ...
                    (
                        <div className="container-fluid">
                            <nav className=" nav1 navbar navbar-expand-lg navbar-light" style={{ backgroundColor: "green" }}>
